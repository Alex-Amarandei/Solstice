...
export function AccountBalanceCheck({ address }: { address: PublicKey }) {
	...
	if (query.isError || !query.data) {
		...
		toast.error(`You are connected to ${cluster.name}, but your account is not found on this cluster. 🔍`, {
			action: {
				label: 'Request Airdrop 🪂',
				onClick: () => {
					mutation.mutateAsync(1).catch((err) => {
						console.error(err);
						toast.error('Airdrop request failed. 😵 Please try again.');
					});
				},
			},
		});
